...
@test(3, "Public: Fixed Priority Test 1")
def public_test1_fp():
    r.run_qemu(
        shell_script(["test1 FP"]),
    ...
    r.match(
        ...
        r"dispatch process#5 at 13: allocated_time=2",
        r"dispatch process#4 at 15: allocated_time=2",
        r"process#4 finish one cycle at 17: 2 cycles left",
        r"dispatch process#5 at 17: allocated_time=1",
        r"process#5 finish one cycle at 18: 2 cycles left",
...
@test(3, "Public: Fixed Priority Test 2")
def public_test2_fp():
    r.run_qemu(
        shell_script(["test2 FP"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        ...
        r"process#5 finish one cycle at 5: 4 cycles left",
        r"dispatch process#4 at 5: allocated_time=1",
        r"process#4 misses a deadline at 6 in scheduler",
        ...
        r"dispatch process#5 at 12: allocated_time=2",
        r"process#5 finish one cycle at 14: 1 cycles left",
        r"dispatch process#4 at 14: allocated_time=1",
        r"process#4 misses a deadline at 15 in scheduler",
...
@test(3, "Public: Fixed Priority Test 3")
def public_test3_fp():
    r.run_qemu(
        shell_script(["test3 FP"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        r"dispatch process#5 at 0: allocated_time=3",
        r"process#5 finish one cycle at 3: 1 cycles left",
        r"dispatch process#6 at 3: allocated_time=1",
        r"process#6 finish one cycle at 4: 2 cycles left",
        r"dispatch process#4 at 4: allocated_time=1",
        r"dispatch process#6 at 5: allocated_time=1",
        r"process#6 finish one cycle at 6: 1 cycles left",
        r"dispatch process#4 at 6: allocated_time=1",
        r"process#4 finish one cycle at 7: 1 cycles left",
...
@test(3, "Public: Fixed Priority Test 4")
def public_test4_fp():
    r.run_qemu(
        shell_script(["test4 FP"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        r"dispatch process#6 at 0: allocated_time=1",
        r"process#6 finish one cycle at 1: 3 cycles left",
        r"dispatch process#4 at 1: allocated_time=2",
        r"process#4 finish one cycle at 3: 4 cycles left",
        r"dispatch process#5 at 3: allocated_time=1",
        r"dispatch process#4 at 4: allocated_time=1",
        r"dispatch process#6 at 5: allocated_time=1",
        r"process#6 finish one cycle at 6: 2 cycles left",
        r"process#5 misses a deadline at 6 in scheduler",
        r"dispatch process#4 at 6: allocated_time=1",
        r"process#4 finish one cycle at 7: 3 cycles left",
        ...
        r"dispatch process#6 at 10: allocated_time=1",
        r"process#6 finish one cycle at 11: 1 cycles left",
        r"dispatch process#5 at 11: allocated_time=1",
        r"process#5 finish one cycle at 12: 1 cycles left",
...
@test(3, "Public: Fixed Priority Test 5")
def public_test5_fp():
    r.run_qemu(
        shell_script(["test5 FP"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        ...
        r"dispatch process#6 at 1: allocated_time=1",
        r"process#6 finish one cycle at 2: 2 cycles left",
        r"dispatch process#5 at 2: allocated_time=1",
        r"process#5 finish one cycle at 3: 4 cycles left",
        r"dispatch process#4 at 3: allocated_time=1",
        r"process#4 misses a deadline at 4 in scheduler",
        r"dispatch process#5 at 4: allocated_time=1",
        r"process#5 finish one cycle at 5: 3 cycles left",
        r"dispatch process#6 at 5: allocated_time=1",
        r"process#6 finish one cycle at 6: 1 cycles left",
        r"dispatch process#5 at 6: allocated_time=1",
        r"process#5 finish one cycle at 7: 2 cycles left",
        r"dispatch process#4 at 7: allocated_time=1",
        r"process#4 misses a deadline at 8 in scheduler",
