...
@test(3, "Public: EFDF Test 1")
def public_test1_efdf():
    r.run_qemu(
        shell_script(["test1 EFDF"]),
...
@test(3, "Public: EFDF Test 2")
def public_test2_efdf():
    r.run_qemu(
        shell_script(["test2 EFDF"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        r"dispatch process#4 at 0: allocated_time=2",
        r"process#4 finish one cycle at 2: 5 cycles left",
        r"process#5 misses a deadline at 2 in scheduler",
        r"dispatch process#4 at 3: allocated_time=2",
        r"process#4 finish one cycle at 5: 4 cycles left",
        r"process#5 misses a deadline at 5 in scheduler",
        r"dispatch process#4 at 6: allocated_time=2",
        r"process#4 finish one cycle at 8: 3 cycles left",
        r"process#5 misses a deadline at 8 in scheduler",
        r"dispatch process#4 at 9: allocated_time=2",
        r"process#4 finish one cycle at 11: 2 cycles left",
        r"process#5 misses a deadline at 11 in scheduler",
        r"dispatch process#4 at 12: allocated_time=2",
        r"process#4 finish one cycle at 14: 1 cycles left",
        r"process#5 misses a deadline at 14 in scheduler",
        r"dispatch process#4 at 15: allocated_time=2",
        r"process#4 finish one cycle at 17: 0 cycles left",
        r"process#5 misses a deadline at 17 in scheduler",
    )

@test(3, "Public: EFDF Test 3")
def public_test3_efdf():
    r.run_qemu(
        shell_script(["test3 EFDF"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
...
@test(3, "Public: EFDF Test 4")
def public_test4_efdf():
    r.run_qemu(
        shell_script(["test4 EFDF"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        ...
        r"dispatch process#5 at 3: allocated_time=2",
        r"process#5 finish one cycle at 5: 2 cycles left",
        r"dispatch process#4 at 5: allocated_time=2",
        r"process#4 finish one cycle at 7: 3 cycles left",
        r"dispatch process#6 at 7: allocated_time=1",
        r"process#6 finish one cycle at 8: 2 cycles left",
        r"dispatch process#4 at 8: allocated_time=2",
        r"process#4 finish one cycle at 10: 2 cycles left",
        r"dispatch process#5 at 10: allocated_time=2",
        r"process#5 finish one cycle at 12: 1 cycles left",
        r"dispatch process#6 at 12: allocated_time=1",
        r"process#6 finish one cycle at 13: 1 cycles left",
        r"dispatch process#4 at 13: allocated_time=2",
        r"process#4 finish one cycle at 15: 1 cycles left",
        r"dispatch process#5 at 15: allocated_time=2",
        r"process#5 finish one cycle at 17: 0 cycles left",
        r"dispatch process#4 at 17: allocated_time=2",
        r"process#4 finish one cycle at 19: 0 cycles left",
        r"dispatch process#6 at 19: allocated_time=1",
        r"process#6 finish one cycle at 20: 0 cycles left",
    )

@test(3, "Public: EFDF Test 5")
def public_test5_efdf():
    r.run_qemu(
        shell_script(["test5 EFDF"]),
        make_args=["CPUS=1", "INJECT=trap"],
        timeout=30
    )
    r.match(
        ...
        r"process#5 finish one cycle at 8: 2 cycles left",
        r"process#6 misses a deadline at 8 in scheduler",
        r"dispatch process#5 at 8: allocated_time=1",
        r"process#5 finish one cycle at 9: 1 cycles left",
        r"dispatch process#4 at 9: allocated_time=2",
        r"process#4 finish one cycle at 11: 0 cycles left",
        r"dispatch process#5 at 11: allocated_time=1",
        r"process#5 finish one cycle at 12: 0 cycles left",
        r"process#6 misses a deadline at 12 in scheduler",
    )
