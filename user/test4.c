...
main(int argc, char *argv[])
{
  ...
  int pid1 = fork();
  if(pid1 == 0){
    set_firm_constraints(getpid(), 4, 5);
    start_periodic_task(2, 4, 5, 2, 0);
    for(;;);
  }

  int pid2 = fork();
  if(pid2 == 0){
    set_firm_constraints(getpid(), 1, 5);
    start_periodic_task(2, 6, 3, 2, 0);
    for(;;);
  }

  int pid3 = fork();
  if(pid3 == 0){
    set_firm_constraints(getpid(), 2, 5);
    start_periodic_task(1, 5, 4, 1, 0);
    for(;;);
  }
